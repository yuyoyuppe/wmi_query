...
namespace std
{
    ...
    {
        typedef WMIClassProperty argument_type;
...
namespace std
{
    ...
    {
        typedef WMIClassDescription argument_type;
        typedef std::size_t result_type;
...
void generate_class_definition(const WMIClassDescription& class_desc, std::ostream& s)
{
  ...
  s << "  WMIProvider::get().query(\"select * from " << class_desc._name << '"' << ", [&](IWbemClassObject* o, const pugi::xml_document& doc) {\n";
...
{
  const auto wmi_classes = build_wmi_classes_descriptions(wmi_service);

  generate_header_preambule(header);
  generate_source_preambule(source);

  auto class_desc = wmi_classes.find(WMIClassDescription{"Win32_FolderRedirectionHealth", {}});
  //for(const auto& class_desc : wmi_classes)
  {
    generate_class_declaration(*class_desc, header);
    generate_class_definition(*class_desc, source);
  }
  class_desc = wmi_classes.find(WMIClassDescription{"Win32_UserProfile", {}});
  {
      generate_class_declaration(*class_desc, header);
      generate_class_definition(*class_desc, source);
  }
...
int main()
{
  ...
  wmi_service.query("Select * from meta_class where __this isa \"Win32_UserProfile\"", build_wmi_class_description);
